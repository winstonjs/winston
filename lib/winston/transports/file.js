  ...
  query(options, callback) {
    ...
    stream.on('data', data => {
      if (options.order === 'asc') {
        ...
        // fs-reverse, data is a complete line
        if (data && (!options.start || row >= options.start)) {
          add(data);
        }
        if (data) {
          row++;
    ...
    function add(buf, attempt) {
      try {
        const log = JSON.parse(buf);
        if (check(log)) {
          push(log);
        }
      } catch (e) {
    ...
    function normalizeQuery(opts) {
      opts = opts || {};

      // limit
      opts.rows = opts.rows || opts.limit || 10;

      // starting row offset
      opts.start = opts.start || 0;

      // now
      opts.until = opts.until || new Date();
      if (typeof opts.until !== 'object') {
        opts.until = new Date(opts.until);
      }

      // now - 24
      opts.from = opts.from || (opts.until - (24 * 60 * 60 * 1000));
      if (typeof opts.from !== 'object') {
        opts.from = new Date(opts.from);
      }

      // 'asc' or 'desc'
      opts.order = opts.order || 'desc';
