...
File.prototype._createStream = function () {
  ...
  (function checkFile (target) {
    ...
    function compressFile() {
      if (self._archive) {
        var fileToArchive = String(self._archive);
        self._archive = '';
        var gzip = zlib.createGzip();

        fs.stat(fileToArchive, function(fileToArchiveStatError, fileToArchiveStatResult) {

          var inp = fs.createReadStream(fileToArchive);
          var out = fs.createWriteStream(fileToArchive + '.gz');

          inp.pipe(gzip).pipe(out);

          fs.stat(fileToArchive, function(fileToDeleteStatError, fileToDeleteStatResult) {
            if (fileToArchiveStatResult.ino === fileToDeleteStatResult.ino)
              fs.unlink(fileToArchive, function () {});
          });
