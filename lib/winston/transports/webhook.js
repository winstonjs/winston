...
Webhook.prototype.log = function (level, msg, meta, callback) {
  ...
  var req = http.request(options, function(res) { }); // no callback on request, fire and forget about the response

  req.on('error', function(e) {
    // TODO: do something more meaningful with this error
    console.log(('Error executing Webhook transport http request: ' + e.message).red);
    req.end(); // <-- Is this req already gaurenteed to be closed?
  });
  ...
  // 
  var jsonMessage = { method: 'log', params: { timestamp: utils.timestamp(), msg: msg, level: level, meta: meta } };
