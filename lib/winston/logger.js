...
Logger.prototype.add = function (transport, options, created) {
  ...
  else if (instance.name !== 'file' && this.transports[instance.name]) {
    throw new Error('Transport already attached: ' + instance.name);
  } 
  else if (instance.name === 'file' && this.transports[instance.name + instance.filename]) {
    throw new Error('Transport already attached: ' + instance.name + ': ' + instance.filename);
  }


  if (instance.name === 'file') {
      this.transports[instance.name + instance.filename] = instance;
  } 
  else {
