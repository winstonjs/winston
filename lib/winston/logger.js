...
class Logger extends Transform {
  /**
     ...
     * @param {Object} [_childOptions] - option overrides
     */
  constructor(options, _childOptions) {
    ...
    if (_childOptions) {
      if (!(options instanceof Logger)) {
        throw new TypeError(
          'first arg must be a parent Logger when childOptions are passed');
      }
      composedOptions = Object.assign({}, options, _childOptions);
      composedOptions.transports = options.transportsParam;
    } else {
      composedOptions = options;
    }
    this.configure(composedOptions);
  }
  ...
  configure({
    ...
    if (transports) {
      ...
      this.transportsParam = transports; // Store this for reuse in child loggers
    }
