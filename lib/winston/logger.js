...
Logger.prototype.log = function (level, msg) {
  ...
  for (var key in this.transports) {
    ...
    if ((transport.level && self.levels[transport.level] <= self.levels[level])
      ...
      // we destyle the ANSI color escape codes for non-console Transports
      //
      ...
      transport.log(level, msg, meta, function (err) {
        if (err) {
          errs.push({ error: err, transport: transport });
        }
        
        if (err && self.emitErrs) {
          return self.emit('error', err, transport);
        }
        
        self.emit('log', transport, level, msg, meta);
        if (++logged == len && callback) {
          callback(errs.length > 0 ? errs : null, level, msg, meta); 
        }
      });
    }
  };
...
Logger.prototype.add = function (transport, options) {
  ...
  if (!name && !transport.prototype.log) {
    throw new Error('Unknown transport with no log() method');
  }
  else if (this.transports[name]) {
    throw new Error('Transport already attached: ' + name);
  }
