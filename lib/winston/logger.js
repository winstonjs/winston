...
var Logger = exports.Logger = function (options) {
  ...
  options = options || {};
  // set Levels
  var defaultLevels =  options.levels ? Levels[options.levels] : winston.clone(winston.levels);
  this.levelType = options.levels ? options.levels : winston.defaultLogLevels;
  this.levels = {};
  this.colors = Levels.colors;
  ...
  Object.keys(defaultLevels).forEach(function (level) {
    self.setLevel(level, {position: defaultLevels[level]});
  });
...
Logger.prototype.setLevel = function (level, options) {
  var self = this;
  ...
  if(position > winston.size(levels)) position = winston.size(levels);
  ...
  //   Define  methods for each log level to the object
...
Logger.prototype.removeLevel = function (level) {
  ...
  if(self.level == level) {
     Object.keys(levels).forEach(function(level) {
       if(levels[level] == position) self.level = level;
     });
  }
