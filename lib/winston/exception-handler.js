...
ExceptionHandler.prototype.handle = function () {
  ...
  if (this.handlers.size) {
    // TODO: What to do if you are calling `.handle`
    // more than once?
  }

  args.forEach(arg => {
    if (Array.isArray(arg)) {
      arg.forEach(handler => this.handlers.add(handler));
      return;
    }

    this.handlers.add(arg);
  });
...
ExceptionHandler.prototype.getAllInfo = function (err) {
  return {
    error:   err,
    message: 'uncaughtException: ' + (err.message || '(no error message)'),
...
ExceptionHandler.prototype._getExceptionHandlers = function () {
  return this.handlers.values().concat(
    this.logger.transports.filter(function (transport) {
